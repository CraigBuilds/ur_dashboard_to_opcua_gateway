...
variables. Method functions are zero-argument commands exposed as OPC UA methods.

The implementation owns synchronous ``asyncua`` setup, Python-to-OPC-UA type conversion, callback adaptation, polling, and resource shutdown. It deliberately
does not model arbitrary folders, objects, node identifiers, security schemes, robot protocols, or application workflow. Consumers requiring those capabilities
can use ``asyncua`` directly instead of expanding this package's bounded API.
